...
export function StationTable({ stations, metrics, onSelectionChange, lang = 'en' }) {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex items-center gap-4 py-2">
                        ...
                        {/* Static Icons mapping based on label keywords if needed, or simple placeholder */}
    ...
    return (
        <div className="bg-transparent h-full flex flex-col min-h-0 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar relative">
                <Table className="relative z-10 border-collapse">
                    <TableHeader className="bg-transparent border-b border-white/5 sticky top-0 z-30 backdrop-blur-md">
                    ...
                    <TableBody>
                        {(() => {
                            ...
                            const renderRows = (rowGroup, titleKey, isStale) => (
                                <>
                                    {rowGroup.length > 0 && (
                                        <TableRow className="bg-white/5 border-none">
                                            <TableCell colSpan={columns.length} className="py-2 px-5 h-8 align-middle">
                                                <div className="flex items-center gap-3">
                                                    <div className={cn("w-1 h-3 rounded-full", isStale ? "bg-rose-500/40" : "bg-primary")} />
                                                    <span className="text-[9px] font-black uppercase tracking-[.3em] text-foreground/30">
                                                        {t(titleKey)} <span className="ml-2 text-primary/30">[{rowGroup.length} {t('nodes')}]</span>
                                                    </span>
                                                </div>
                                            </TableCell>
                                        </TableRow>
                                    ...
                                    {rowGroup.map((row) => (
                                        ...
                                        >
                                            {row.getVisibleCells().map((cell) => (
                                                <TableCell key={cell.id} className="px-5 py-0 h-12 border-none">
                            ...
                            return (
                                <>
                                    {renderRows(activeRows, "active", false)}
                                    {renderRows(staleRows, "stale", true)}
                                </>
                            );
                        })()}
                    </TableBody>
